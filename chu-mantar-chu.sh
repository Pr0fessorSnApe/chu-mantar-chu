...
# Version: 2.2 (Bugfix: Selection Logic & Parsing)
# ==============================================================================
...
display_banner() {
    ...
    echo "  ╔══════════════════════════════════════════════════════╗"
    echo "  ║                🪄  CHU-MANTAR-CHU  🪄                 ║"
    echo "  ║        Professional WiFi Security Auditor v2.2         ║"
    echo "  ╚══════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
scan_and_select() {
    ...
    if [[ ! -f "${scan_file}-01.csv" ]]; then
        log_error "Fatal: Could not generate scan results."
        exit 1
    fi
    ...
    # Improved parsing: Get lines between BSSID header and Station header
    local raw_data=$(sed -n '/BSSID,/,/Station/p' "${scan_file}-01.csv" | grep -v "BSSID," | grep -v "Station" | grep ":")
    ...
    while IFS=',' read -r bssid first_time last_time channel speed privacy cipher auth power beacons ivs lan_ip id_len essid key; do
        # Clean data
